...
def test_byte_cap_truncates_by_bytes() -> None:
    ...
    result = truncate(['x' * 1024 for _ in range(100)])
...
def test_giant_first_line_flagged_and_omitted() -> None:
    result = truncate(['x' * (DEFAULT_MAX_BYTES + 1), 'rest'])
...
def test_truncated_property_tracks_truncated_by() -> None:
    assert truncate(['a']).truncated is False
    assert truncate(['x'] * (DEFAULT_MAX_LINES + 1)).truncated is True
