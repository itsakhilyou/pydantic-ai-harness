...
from pydantic_ai_harness.planning import PlanItem, PlanningToolset, TaskStatus
...
class TestPlanningCapability:
    ...
    def test_plan_property_returns_copy(self) -> None:
        cap = Planning[None]()
        cap._state.items.append(PlanItem(content='x'))
        snapshot = cap.plan
        snapshot.clear()
        assert len(cap.plan) == 1
    ...
    async def test_for_run_isolates_state_and_preserves_config(self) -> None:
        ...
        assert len(cap.plan) == 1  # original untouched
