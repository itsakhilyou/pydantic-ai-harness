...
| [Staleness Tracker](staleness.md) | Tells the model when files it read have changed on disk since it read them, via an ephemeral per-request notice that never enters durable history. | -- |
