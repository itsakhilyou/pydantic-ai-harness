...
async def _run_ripgrep(root: Path, target: Path, pattern: str) -> list[AbstractMatch]:
    ...
    return _parse_ripgrep_json(stdout, root)


def _parse_ripgrep_json(stdout: bytes, root: Path) -> list[AbstractMatch]:
    ...
    matches LocalEnvironment's previous behavior.
    """
...
class LocalEnvironment(AbstractEnvironment):
    ...
    *advisory only* -- it guards against accidental escapes and reduces noise, but it is
    ...
    async def grep(self, path: str, pattern: str) -> list[AbstractMatch]:
        ...
        return await _run_ripgrep(root, resolved_path, pattern)
