...
from ._ripgrep import run_ripgrep, run_ripgrep_files
...
class LocalEnvironment(AbstractEnvironment):
    ...
    *advisory only* -- it guards against accidental escapes and reduces noise, but it is
    ...
    async def grep(self, path: str, pattern: str) -> list[AbstractMatch]:
        ...
        # read_file/ls use) because rg's "no such file" comes back as exit code 2 + a generic
        # error string; we get a cleaner contract by checking up front.
        ...
        # rg handles both single files and directory walks natively; no need for our own
        # is_file/is_dir branching like the old Python re path required. Binary file skipping,
        # gitignore honoring, and line numbering are all rg's job. Determinism (sort) is
        # added at the capability layer, same as `ls` -- backends stay thin.
        return await run_ripgrep(root, resolved_path, pattern)
    ...
    async def glob(self, path: str, pattern: str) -> list[str]:
        ...
        # `rg --files -g <pattern>` is the single engine the ABC promises across backends; bare
        # `*.py` matches at any depth (gitignore semantics), dotfiles are excluded by rg's default
        # hidden-skip, and the conformance suite verifies one engine instead of policing two.
        return await run_ripgrep_files(root, resolved_path, pattern)
