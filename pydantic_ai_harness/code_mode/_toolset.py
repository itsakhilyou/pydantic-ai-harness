...
- **No wall-clock or timing primitives**: `asyncio.sleep`, `datetime.datetime.now()`, `datetime.date.today()`, and the `time` module are unavailable. Don't insert sleeps, delays, or rate-limiting between tool calls -- they aren't needed inside the sandbox. Don't filter on "current time" / "last N hours" / "today"; rely on fields the tools return instead.
